...
| **[caching/INDEX.md](caching/INDEX.md)** | Adds or changes a cache — `@Cacheable`/`@CacheEvict`, Caffeine/Guava/Ehcache/Redis/Hazelcast, a cache key, TTL, eviction rule, invalidation path, or warm-up — **or** adds a de facto cache: a static `Map`, a memoised field, reference data loaded once at startup. Its own dispatch index; read that, not all twelve files. |
