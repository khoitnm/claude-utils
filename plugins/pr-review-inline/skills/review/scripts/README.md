...
| No project-specific identifiers (ticket keys, private wiki/tracker links) | The checklists have to work on a repo nobody here has seen | error |
...
The identifier check only knows shapes that give themselves away anywhere — a
ticket key, a link to a wiki only your team can open. Your employer's name, its
repo and module prefixes have the same problem and no recognisable shape, so
they go in a list that is *not* committed (adding them here would publish them):

```bash
# one case-insensitive regex per line, # comment lines allowed
printf '%s\n' 'acme' '\bacme-(api|web)\b' > .skill-lint-private-terms

# or inline, for CI, os.pathsep-separated (: on POSIX, ; on Windows)
SKILL_LINT_PRIVATE_TERMS='acme:\bacme-(api|web)\b' python scripts/lint-skill-docs.py
```

`.skill-lint-private-terms` is gitignored at the repo root. The file is read
from the linter's root argument, so keep it next to `SKILL.md`.
