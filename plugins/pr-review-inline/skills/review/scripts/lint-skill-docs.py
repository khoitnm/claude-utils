...
# Identifiers that mean a specific repo, module, tracker, or ticket leaked into a
# checklist that is supposed to work on any codebase. Only shapes that give
# themselves away on any team belong here: blocking a name that is private to one
# org would mean writing that name into this public file, which publishes exactly
# what it is trying to keep out. Those go in PRIVATE_TERMS_FILE instead.
PROJECT_SPECIFIC = [
    ...
    r"[A-Z]{2,5}-\d{3,6}\b",
    # a wiki or tracker nobody outside the team can open
    r"\b(?:confluence|jira|wiki)\.[\w.-]+/",
    r"\b[\w.-]+\.atlassian\.net\b",
]

# Names too private to commit — an employer, its repos, its module prefixes — as
# one regex per line, `#` comment lines allowed. Both sources are meant to stay
# out of git; the file is listed in .gitignore.
PRIVATE_TERMS_FILE = ".skill-lint-private-terms"
PRIVATE_TERMS_ENV = "SKILL_LINT_PRIVATE_TERMS"
...
def identifier_patterns(root):
    """The shape-based patterns above, plus whatever private terms are configured.

    The env var is os.pathsep-separated so a CI job can set it inline; the file
    is for a checkout where the same terms apply to every run.
    """
    patterns = [re.compile(p) for p in PROJECT_SPECIFIC]
    raw = os.environ.get(PRIVATE_TERMS_ENV, "").split(os.pathsep)
    path = os.path.join(root, PRIVATE_TERMS_FILE)
    if os.path.isfile(path):
        raw += open(path, encoding="utf-8").read().split("\n")
    for term in (t.strip() for t in raw):
        if not term or term.startswith("#"):
            continue
        try:
            patterns.append(re.compile(term, re.IGNORECASE))
        except re.error as exc:
            print(f"{PRIVATE_TERMS_FILE}: skipping bad regex {term!r}: {exc}",
                  file=sys.stderr)
    return patterns
...
def main():
    ...
    total = 0
    table = []
    id_patterns = identifier_patterns(root)
